...
export const CategoryCard = ({ name, image, slug, categoryId, topSellers }: Props) => {
  ...
  return (
    <Container>
      <Card className="text-center h-100 shadow-sm border-0 justify-content-center category-card">
        <Link to={`/urunler?main_category_id=${categoryId || slug || name.toLowerCase()}`} className="text-decoration-none">
          <Card.Img
            variant="top"
            src={fullImageUrl}
            alt={name}
